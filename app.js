(() => {
  ...
  function sayPinball(text, force){ if(!pinballCaption) return; const a=state?.aurora; if(!force && a && (a.forced||a.autoLoop)) return; if(text===FORCE_ZALGO){ pinballGame?.classList.add("zalgo-active"); pinballCaption.classList.add("zalgo-original"); pinballCaption.textContent=FORCE_ZALGO; return; } pinballGame?.classList.remove("zalgo-active"); pinballCaption.classList.remove("zalgo-original"); pinballCaption.textContent=text; }
